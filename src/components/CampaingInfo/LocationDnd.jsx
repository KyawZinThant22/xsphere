...
const LocationDnd = ({ onChange, item }) => {
  const inputFocusColor = " focus:border-[#61C09D]";
  // Demo API
  const [demoList, setDemoList] = useState(item);
  ...
  const addLocationToInputBox = useCallback(
    (data) => {
  ...
  useEffect(() => {
    const filter = demoList.filter((el) => el.default === true);
    setDefaultList(filter);
  }, []);
  return (
    <div className="mt-2 mb-3">
      ...
      <div className="mt-1 relative">
        <div
          ...
          style={{
            padding:
              value.length > 0 || defaultList.length > 0
                ? "0.25rem 0.75rem"
                : "0.60rem 0.75rem",
          }}>
          <ReactSortable
